import random
import numpy as np
from full_conected.activators import SigmoidActivator, IdentityActivator
from full_conected.full_connect_oo import train_data_set
...
class FullConnectedLayer(object):
    def __init__(self, input_size, output_size, activator):
        """
        构造函数
        :param input_size: 本层输入向量的维度(上层节点数)
        :param output_size: 本层输出向量的维度(本层节点数)
        :param activators: 激活函数
        """
...
class Network(object):
    def __init__(self, layers):
        """
        构造函数
        :param layers: 一个向量，长度代表层数，每个元素数值代表该层节点数
        """
        self.layers = []
